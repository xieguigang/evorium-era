    ...
    Public Class NutrientReplenishmentRule : Implements IEnvironmentRule
                    ...
                    For z As Integer = 0 To dims.Depth - 1
                        ...
                        If delta <> 0 Then
                            If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.Oxygen) Then
                                voxel.ExternalMolecules(MoleculeType.Oxygen).SetQuantity(0)
                            End If

                            voxel.ExternalMolecules(MoleculeType.Oxygen).SetQuantity(Math.Max(0, currentOxygen + delta))
                        End If
                    Next
                Next
            Next
            ...
            For Each pos In hotspots
                If env.IsValidCoordinate(pos.X, pos.Y, pos.Z) Then
                    ...
                    If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.CarbonSource) Then
                        voxel.ExternalMolecules(MoleculeType.CarbonSource).SetQuantity(0)
                    End If
                    If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.NitrogenSource) Then
                        voxel.ExternalMolecules(MoleculeType.NitrogenSource).SetQuantity(0)
                    End If

                    ' 补充碳源/补充氮源
                    voxel.ExternalMolecules(MoleculeType.CarbonSource).AddQuantity(config.NutrientHotspotStrength)
                    voxel.ExternalMolecules(MoleculeType.NitrogenSource).AddQuantity(config.NutrientHotspotStrength \ 2)

                    ' [v3.0] 热点也补充glucose
                    If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.Glucose) Then
                        voxel.ExternalMolecules(MoleculeType.Glucose) = Molecule.EmptyModel(MoleculeType.Glucose)
                    End If
                    voxel.ExternalMolecules(MoleculeType.Glucose).AddQuantity(config.NutrientHotspotStrength \ 3)
                End If
            Next
        End Sub
        ...
        Private Sub ReplenishGlobalNutrients(env As NaturalEnvironment)
            ...
            For i As Integer = 0 To 9
                ...
                If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.CarbonSource) Then
                    voxel.ExternalMolecules(MoleculeType.CarbonSource) = Molecule.EmptyModel(MoleculeType.CarbonSource)
                End If
                If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.NitrogenSource) Then
                    voxel.ExternalMolecules(MoleculeType.NitrogenSource) = Molecule.EmptyModel(MoleculeType.NitrogenSource)
                End If
                If Not voxel.ExternalMolecules.ContainsKey(MoleculeType.Water) Then
                    voxel.ExternalMolecules(MoleculeType.Water) = Molecule.EmptyModel(MoleculeType.Water)
                End If

                voxel.ExternalMolecules(MoleculeType.CarbonSource).AddQuantity(config.NutrientReplenishmentCarbon \ 10)
                voxel.ExternalMolecules(MoleculeType.NitrogenSource).AddQuantity(config.NutrientReplenishmentNitrogen \ 10)
                voxel.ExternalMolecules(MoleculeType.Water).AddQuantity(5)
            Next
        End Sub
        ...
        Private Sub EnsureMolecule(voxel As Container.Voxel, type As MoleculeType, minAmount As Integer)
            If Not voxel.ExternalMolecules.ContainsKey(type) Then
                voxel.ExternalMolecules(type) = Molecule.EmptyModel(type)
            End If
            If voxel.ExternalMolecules(type) < minAmount Then
                voxel.ExternalMolecules(type).SetQuantity(minAmount)
            End If
        End Sub
